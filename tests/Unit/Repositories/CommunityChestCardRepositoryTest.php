...
class CommunityChestCardRepositoryTest extends TestCase
{
    ...
    public function test_assign_card_to_player_sets_holder_join_order_on_pivot_row(): void
    {
        ...
        $cardId = DB::table('game_community_chest_cards')
            ->where('game_id', $game->id)
            ->orderBy('sort_order')
            ->value('community_chest_card_id');

        $this->repository->assignCardToPlayer($game->id, (int) $cardId, 3);

        $holderJoinOrder = DB::table('game_community_chest_cards')
            ->where('game_id', $game->id)
            ->where('community_chest_card_id', $cardId)
            ->value('holder_join_order');

        $this->assertSame(3, $holderJoinOrder);
    }
    ...
    public function test_get_held_cards_for_game_groups_cards_by_holder_join_order(): void
    {
        ...
        $cardIds = DB::table('game_community_chest_cards')
            ->where('game_id', $game->id)
            ->orderBy('sort_order')
            ->limit(2)
            ->pluck('community_chest_card_id')
    ...
    public function test_release_card_from_player_clears_holder_and_moves_card_to_bottom(): void
    {
        ...
        $pivotRow = DB::table('game_community_chest_cards')
            ->where('game_id', $game->id)
            ->where('community_chest_card_id', $cardId)
